...
class DashboardBase(QWidget):
    ...
    def criar_grade_metricas(self, metricas):
        ...
        for indice, metrica in enumerate(metricas):
            ...
            card.setProperty("tipo", metrica.get("tipo", "padrao"))
            ...
            descricao = QLabel(metrica.get("descricao", ""))
            descricao.setObjectName("descricaoMetrica")
            ...
            grade.addWidget(card, indice // 4, indice % 4)
    ...
    def criar_secao(self, titulo, subtitulo=""):
        ...
        label_titulo.setObjectName("tituloSecaoDashboard")
        cabecalho.addWidget(label_titulo)

        if subtitulo:
            label_subtitulo = QLabel(subtitulo)
            label_subtitulo.setObjectName("subtituloSecaoDashboard")
    ...
    def criar_acoes(self, acoes):
        frame, layout = self.criar_secao(
            "Acessos rápidos",
            "Abra as áreas mais usadas sem procurar entre as abas.",
        )
        linha = QHBoxLayout()
        linha.setSpacing(10)

        for titulo, chave, principal in acoes:
            botao = QPushButton(titulo)
            botao.setObjectName(
                "botaoAcaoPrincipal" if principal else "botaoAcaoDashboard"
            )
            botao.clicked.connect(
                lambda checked=False, destino=chave: self.navegar.emit(destino)
            )
    ...
    def criar_linha_info(self, titulo, detalhe, status=""):
        linha = QFrame()
        ...
        label_titulo = QLabel(titulo)
        label_titulo.setObjectName("tituloLinhaDashboard")
        label_titulo.setWordWrap(True)

        label_detalhe = QLabel(detalhe)
        label_detalhe.setObjectName("detalheLinhaDashboard")
    ...
    def mostrar_erro(self, erro):
        frame, layout = self.criar_secao("Não foi possível carregar o painel")
        mensagem = QLabel(str(erro))
        mensagem.setObjectName("mensagemErroDashboard")
...
class TelaDashboardUsuario(DashboardBase):
    def __init__(self, usuario):
        nome = getattr(usuario, "nome", "Usuário") or "Usuário"
        super().__init__(
            usuario,
            f"Olá, {nome}!",
            "Veja seus pedidos, consultas e os próximos passos da sua conta AFGMED.",
        )
        self.recarregar()
    ...
    def recarregar(self):
        ...
        try:
            with app.app_context():
                ...
                itens_carrinho = sum(
                    int(item.quantidade or 0) for item in (carrinho.itens if carrinho else [])
                )

                pedidos_pendentes = Pedido.query.filter(
                    Pedido.id_usuario == self.usuario_id,
                    Pedido.status.in_(["aguardando_pagamento", "falha"]),
                ).count()
                ...
                proxima = consultas_futuras[0] if consultas_futuras else None
                ...
                dados_pedidos = [
                    {
                        ...
                        "status": pedido.status or "em análise",
                    }
                ...
                if proxima:
                    dados_proxima = {
                        "medico": (
                            f"{proxima.medico.nome} {proxima.medico.sobrenome}"
                        ).strip(),
                        "especialidade": proxima.medico.especialidade or "",
                        "data": proxima.data,
            ...
            metricas = [
                {
                    ...
                    "descricao": "Produtos aguardando finalização.",
                    "tipo": "azul",
                },
                {
                    "titulo": "Pedidos pendentes",
                    "valor": pedidos_pendentes,
                    "descricao": "Aguardando pagamento ou nova tentativa.",
                    "tipo": "amarelo",
                },
                {
                    "titulo": "Próximas consultas",
                    "valor": len(consultas_futuras),
                    "descricao": "Agendamentos futuros confirmados.",
                    "tipo": "verde",
                },
                {
                    "titulo": "Último pedido",
                    "valor": f"#{dados_pedidos[0]['id']}" if dados_pedidos else "—",
                    "descricao": (
                        formatar_real(dados_pedidos[0]["total"])
                        if dados_pedidos
                        else "Nenhum pedido realizado."
                    ),
                    "tipo": "cinza",
                },
            ]
            self.area_dinamica.addLayout(self.criar_grade_metricas(metricas))

            self.area_dinamica.addWidget(
                self.criar_acoes(
                    [
                        ("Ver produtos", "produtos", True),
                        ("Abrir carrinho", "carrinho", False),
                        ("Agendar consulta", "consultas", False),
                        ("Meus pedidos", "pedidos", False),
                    ]
                )
            )
            ...
            proxima_frame, proxima_layout = self.criar_secao(
                "Próxima consulta",
                "Seu compromisso médico mais próximo.",
            )
            if dados_proxima:
                proxima_layout.addWidget(
                    self.criar_linha_info(
                        ...
                        (
                            ...
                            f"{formatar_data(dados_proxima['data'])} às "
                            f"{dados_proxima['horario']}"
                        ),
                        "Agendada",
                    )
                )
                ...
                proxima_layout.addWidget(
                    self.criar_linha_info(
                        ...
                        "Escolha um médico para realizar um novo agendamento.",
                    )
                )

            pedidos_frame, pedidos_layout = self.criar_secao(
                "Pedidos recentes",
                "Acompanhe as últimas compras registradas.",
            )
            if dados_pedidos:
                for pedido in dados_pedidos:
                    pedidos_layout.addWidget(
                        self.criar_linha_info(
                            f"Pedido #{pedido['id']} • {formatar_real(pedido['total'])}",
                            f"Criado em {pedido['data'].strftime('%d/%m/%Y %H:%M')}",
                ...
                pedidos_layout.addWidget(
                    self.criar_linha_info(
                        ...
                        "Seus pedidos aparecerão aqui após a finalização do carrinho.",
                    )
                )

            grade.addWidget(proxima_frame, 0, 0)
            grade.addWidget(pedidos_frame, 0, 1)
            grade.setColumnStretch(0, 1)
            grade.setColumnStretch(1, 1)
            self.area_dinamica.addLayout(grade)
...
class TelaDashboardMedico(DashboardBase):
    def __init__(self, usuario):
        nome = getattr(usuario, "nome", "Médico") or "Médico"
        super().__init__(
            usuario,
            f"Painel médico — Dr(a). {nome}",
            "Acompanhe sua agenda, os pacientes do dia e o histórico de atendimentos.",
        )
        self.recarregar()
    ...
    def recarregar(self):
        ...
        if not getattr(self.usuario, "id_medico", None):
            frame, layout = self.criar_secao("Cadastro médico não vinculado")
            layout.addWidget(
                self.criar_linha_info(
                    "Seu usuário ainda não possui um médico associado",
                    "Peça ao administrador para vincular este acesso a um cadastro médico.",
                )
            )
            self.area_dinamica.addWidget(frame)
            return
        ...
        try:
            with app.app_context():
                medico = Medico.query.get(self.usuario.id_medico)
                consultas = (
                    Consulta.query.filter_by(medico_id=self.usuario.id_medico)
                    .order_by(Consulta.data.asc(), Consulta.horario.asc())
                    .all()
                )
                ...
                hoje_agendadas = [
                    ...
                    if consulta.data == hoje and consulta.status == "agendada"
                ]
                futuras = [
                    consulta
                    for consulta in consultas
            ...
            metricas = [
                ...
                },
                {
                    "titulo": "Próximas consultas",
                    "valor": len(futuras),
                    "descricao": "Agenda futura ainda aberta.",
                    "tipo": "verde",
                },
                {
                    "titulo": "Concluídas",
                    "valor": len(concluidas),
                    "descricao": "Atendimentos finalizados no sistema.",
                    "tipo": "cinza",
                },
                {
                    "titulo": "Canceladas",
                    "valor": len(canceladas),
                    "descricao": "Consultas canceladas pelos usuários.",
                    "tipo": "vermelho",
                },
            ]
            self.area_dinamica.addLayout(self.criar_grade_metricas(metricas))
            ...
            self.area_dinamica.addWidget(
                self.criar_acoes(
                    [
                        ("Abrir minha agenda", "consultas_medico", True),
                        ("Ver produtos", "produtos", False),
                        ("Meu perfil", "perfil", False),
                    ]
                )
            )
            ...
            perfil_frame, perfil_layout = self.criar_secao(
                "Identificação profissional",
                "Dados vinculados ao seu acesso médico.",
            )
            perfil_layout.addWidget(
                self.criar_linha_info(
                    nome_medico,
                    especialidade or "Especialidade não informada",
                    "Médico",
                )
            )

            agenda_frame, agenda_layout = self.criar_secao(
                "Próximos pacientes",
                "Ordem cronológica dos atendimentos agendados.",
            )
            if proximas:
                    ...
                    agenda_layout.addWidget(
                        self.criar_linha_info(
                            ...
                            (
                                ...
                                f"{formatar_data(consulta['data'])} às {consulta['horario']}"
                            ),
                            "Agendada",
                        )
                    )
                ...
                agenda_layout.addWidget(
                    self.criar_linha_info(
                        "Nenhuma consulta futura",
                        "Novos agendamentos aparecerão automaticamente nesta lista.",
                    )
                )

            grade.addWidget(perfil_frame, 0, 0)
            grade.addWidget(agenda_frame, 0, 1)
            grade.setColumnStretch(0, 1)
...
class TelaDashboardAdmin(DashboardBase):
    def __init__(self, usuario):
        nome = getattr(usuario, "nome", "Administrador") or "Administrador"
        super().__init__(
            usuario,
            f"Painel administrativo — {nome}",
            "Visão geral da operação AFGMED no banco compartilhado entre web e desktop.",
        )
        self.recarregar()
    ...
    def recarregar(self):
        ...
        try:
            with app.app_context():
                ...
                consultas_agendadas = Consulta.query.filter(
                    Consulta.status == "agendada",
                    Consulta.data >= date.today(),
                ).count()
                ...
                pedidos_recentes = (
                    Pedido.query.order_by(Pedido.data_criacao.desc())
                    .limit(6)
                    .all()
                )
                ...
                lista_pedidos = [
                    {
                        "id": pedido.id,
                        "cliente": (
                            f"{pedido.usuario.nome} {pedido.usuario.sobrenome}"
                        ).strip(),
                        "total": float(pedido.total or 0),
                        "status": pedido.status or "em análise",
                    }
            ...
            metricas = [
                ...
                },
                {
                    "titulo": "Médicos",
                    "valor": total_medicos,
                    "descricao": "Profissionais disponíveis.",
                    "tipo": "verde",
                },
                {
                    "titulo": "Produtos ativos",
                    "valor": produtos_ativos,
                    "descricao": f"{estoque_baixo} com estoque crítico.",
                    "tipo": "amarelo" if estoque_baixo else "cinza",
                },
                {
                    "titulo": "Consultas futuras",
                    "valor": consultas_agendadas,
                    "descricao": "Agendamentos ainda em aberto.",
                    "tipo": "verde",
                },
                {
                    "titulo": "Pedidos pendentes",
                    "valor": pedidos_pendentes,
                    "descricao": "Aguardando pagamento ou correção.",
                    "tipo": "vermelho" if pedidos_pendentes else "cinza",
                },
            ]
            self.area_dinamica.addLayout(self.criar_grade_metricas(metricas))
            ...
            self.area_dinamica.addWidget(
                self.criar_acoes(
                    [
                        ("Gerenciar produtos", "admin_produtos", True),
                        ("Gerenciar médicos", "admin_medicos", False),
                        ("Pedidos gerais", "admin_pedidos", False),
                        ("Agenda geral", "admin_consultas", False),
                    ]
                )
            )
            ...
            estoque_frame, estoque_layout = self.criar_secao(
                "Atenção ao estoque",
                "Produtos ativos com até 10 unidades disponíveis.",
            )
            if lista_estoque:
                for produto in lista_estoque:
                    status = "Sem estoque" if produto["estoque"] <= 0 else "Estoque baixo"
                    estoque_layout.addWidget(
                        self.criar_linha_info(
                            produto["nome"],
                            f"Quantidade disponível: {produto['estoque']} unidade(s)",
                            status,
                        )
                    )
            else:
                estoque_layout.addWidget(
                    self.criar_linha_info(
                        ...
                        "Todos os produtos ativos possuem mais de 10 unidades.",
                        "Normal",
                    )
                )
            ...
            pedidos_frame, pedidos_layout = self.criar_secao(
                "Pedidos recentes",
                "Últimas movimentações de compra registradas.",
            )
            if lista_pedidos:
                ...
                pedidos_layout.addWidget(
                    self.criar_linha_info(
                        ...
                        "As compras dos usuários aparecerão nesta área.",
                    )
                )

            grade.addWidget(estoque_frame, 0, 0)
            grade.addWidget(pedidos_frame, 0, 1)
            grade.setColumnStretch(0, 1)
            grade.setColumnStretch(1, 1)
            self.area_dinamica.addLayout(grade)
...
def criar_dashboard(usuario):
    ...
    if bool(getattr(usuario, "is_medico", False)):
